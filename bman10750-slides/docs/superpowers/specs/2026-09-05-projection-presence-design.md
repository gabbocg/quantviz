...
- Sim-slide chrome tightened so the budgets in §4.3 hold: `.reveal section.sim-slide > h2::after { margin-top: 0.4em }`; `.sim-slide .slide-kicker { margin: 0.2em 0 0.4em }`; `.sim-card { margin: 0.25em 0 0.15em }`; `.try-this { margin-top: 0.35em; padding-top: 8px; padding-bottom: 8px }`; `.sim-slide .lede-min { font-size: 26px; margin: 0.25em 0 0.4em }`; `.sim-card.sim-plot .qwebr-output-code-area { min-height: 0; margin-top: 6px }`; `.sim-card { padding: 10px 18px 10px }` (was 14/12); the printed-output `pre` gets `margin: 0; width: 100%; padding-top: 8px; padding-bottom: 8px` because theme.scss's block margin and 90% width are dead space inside the output box. Measured after implementation: a 12-line plot cell with a two-line printed result and a two-line prompt is the tallest sim slide and fits 700px only with these trims.
- `.try-this`: 23px.
