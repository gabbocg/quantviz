...
- Sim-slide chrome tightened so the budgets in §4.3 hold: `.reveal section.sim-slide > h2::after { margin-top: 0.4em }`; `.sim-slide .slide-kicker { margin: 0.2em 0 0.4em }`; `.sim-card { margin: 0.25em 0 0.15em }`; `.try-this { margin-top: 0.35em; padding-top: 8px; padding-bottom: 8px }`; `.sim-slide .lede-min { font-size: 26px; margin: 0.25em 0 0.4em }`; `.sim-card.sim-plot .qwebr-output-code-area { min-height: 0; margin-top: 6px }`; `.sim-card { padding: 8px 18px 8px; margin: 0.1em 0 0.05em }` (was 14/12 and 0.4/0.2em); `.try-this { margin-top: 0.1em }`; the printed-output `pre` gets `margin: 0; width: 100%; padding-top: 8px; padding-bottom: 8px; box-sizing: border-box` because Quarto's reveal theme gives every `pre` a block margin, 90% width and side padding that, on a 100%-wide element with the default content box, overflowed the card and forced a permanent horizontal scrollbar. Measured live after implementation (Task 25b): a 12-line plot cell with a two-line printed result and a two-line prompt is the tallest sim slide (712px before the last trims) and fits 700px only with these values.
- `.try-this`: 23px.
- `.sem-mark::before`: 4em with line-height 0.8, 18% alpha (4.4em in the design; trimmed in Task 5b so every hero fits 700px). `.reveal section.hero > p`: 1.3em. `.reveal section.hero .lede { color: var(--ink) }`.
