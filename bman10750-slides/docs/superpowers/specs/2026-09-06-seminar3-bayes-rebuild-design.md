...
- `.claim-pair`, both cells `.fragment` (first fragment reveals both):
...
- `.slide-footer`, second fragment: the psychologists' complaint is that the base rate never entered anyone's head. The argument since has been about whether 20 to 1 is the right prior here; it has never been about what to do with a prior once you have one. That is the next three slides.

The claim-cell tags reuse the existing `.claim-tag` style but neither cell is "wrong" or "right"; both use a neutral navy tag variant `.claim-neutral` (one new rule in `seminars.scss`).

### 4.3 Grid stage

`assets/js/bayes-grid-anim.html` replaces `bayes-anim.html`. Canvas 1000×445 at the standard wrap. Grid of 21×10 cells, 24 units square on a 27-unit pitch (564×267), left edge at x = 218, top at y = 70; caption at (40, 34) in the `caption` role.
...
| 4 | the 24 survivors slide into one row under the grid (y = 372), 4 navy then 20 grey, with brackets "4 librarians" and "24 fit"; readout at y = 425 in the `readout` role: `P(librarian | description) = 4 / 24 = 16.7%` | the answer is a ratio |

Motion: cells move with `outCubic` over 620ms and a 12ms stagger, as the current file does. `.slide-footer`: almost everyone says librarian; the description does fit librarians four times better, and Steve is still four times more likely to be a farmer, because there are twenty times as many of them. Both facts are in the picture at once.

### 4.4 Square stage

`assets/js/bayes-square-anim.html`, new. Canvas 1000×445. The square is 340×340 at x = 330..670, y = 60..400; the region to its right (x = 700..980) carries the readout; the region below (y = 400..445) carries the width bracket.

| state | move | caption |
|---|---|---|
| 0 | the grid exactly as the previous slide's state 2 (navy column, orange rings), same geometry, so the cut between slides shows no change | the same 210 people |
| 1 | **melt**: every cell tweens `x, y, width, height` into its tile of the square: column c becomes a strip of width 340/21 ≈ 16.2 at x = 330 + c·16.2; row r becomes a band of height 34 at y = 60 + r·34; gaps close; the navy strip is now visibly thin | drawn as area: the librarian strip is 1/21 wide |
| 2 | **shade**: inside every column the ringed tiles slide to the bottom of the strip and fuse into one orange block (rings disappear, fill turns orange): height 4 tiles = 0.40 of the navy strip, 1 tile = 0.10 of each grey strip. The grey blocks fuse sideways into one band 20 strips wide | the likelihoods are heights: 0.40 and 0.10 |
| 3 | **cut away**: unshaded tiles fade to 8%; brackets appear: under the navy strip "P(L) = 1/21", to the right of the navy block "0.40", to the right of the grey band "0.10" | throw away everyone who does not fit |
| 4 | **read**: the readout assembles at right: `navy area / all shaded`, then `(1/21 × 0.40) / (1/21 × 0.40 + 20/21 × 0.10)`, then `= 4/24 = 16.7%` | the posterior is the navy share of what is left |
...
`assets/js/bayeq-anim.html` rewritten (same file name, same stage id `bayeq-stage`, fragments now five). Canvas 1000×445. Left: the square in its state-3 form scaled to 260×260 at x = 60..320, y = 90..350 with the three brackets. Right, x = 380..980: the formula in the `readout` role (mono 30), as a fraction:
...
with the arithmetic line beneath in the `label` role: `0.0190 / (0.0190 + 0.0952) = 0.167 = 4/24`, assembled term by term.
...
n <- 300000
lib  <- runif(n) < prior
fits <- runif(n) < ifelse(lib, fit_lib, fit_farm)
round(c(prior = prior,
        posterior = mean(lib[fits])), 3)
```

Eight lines, longest 48 characters, prints two numbers (0.048 and about 0.167). Kicker: "Seminar 3 · two knobs". `.try-this`: set `fit_farm <- 0.40` and run: the description now says nothing about profession and the posterior lands back on the prior. That is the worksheet's independence test, P(E | H) = P(E), seen from the other side. Put it back, then move the prior to 0.50 and watch the same evidence give 0.80.
...
- `.claim-pair`, cells revealed by one fragment:
...
| 1 | heights are P(upgrade \| buy) = 0.40 and P(upgrade \| not) = 0.10 |
...
| `_quarto.yml` | `include-after-body`: `bayes-data.html` before the three Steve stages; `bayes-anim.html` removed; `bayes-square-anim.html` added |
| `assets/seminars.scss` | `.claim-neutral` tag variant (navy tint), and the `.claim-cell.fragment` opacity rule if Reveal's default fragment styling does not already apply to it |
| `scripts/check-render.sh` | slide ids, stage ids and fragment ids updated per the table in §4 |
...
- Fragment walk forward and backward on `bayes-grid`, `bayes-square`, `bayeq-formula` and `cond-intuition`; the cut from slide 3 to slide 4 shows no visible jump at state 0; the melt and shade play under normal motion and snap under `prefers-reduced-motion`.
