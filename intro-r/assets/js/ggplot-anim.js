...
(function () {
  ...
  // Geometry (stage sub-box) — matches #gg-plot in theme.scss (640x460)
  var W = 640, H = 460;
  var M = { l: 62, r: 130, t: 46, b: 62 };
  ...
  function buildPoints(svg) {
    if (GEOM === 'ohlc') {
      // geom_barchart from tidyquant: high–low wick, left tick = open, right = close.
      var ohlc = window.GG_OHLC || [];
      var upColor = window.GG_UP_COLOR   || '#325D88';
      var dnColor = window.GG_DOWN_COLOR || '#B94A48';
      var xs = ohlc.map(function (r) { return sx(r[0]); });
      var span = ohlc.length > 1 ? Math.abs(xs[1] - xs[0]) : 6;
      var tick = Math.max(1.5, span * 0.4);
      ohlc.forEach(function (r) {
        var x = sx(r[0]);
        var yo = sy(r[1]), yh = sy(r[2]), yl = sy(r[3]), yc = sy(r[4]);
        var color = r[4] >= r[1] ? upColor : dnColor;
  ...
  function buildSmooth(svg) {
    // Explicit overlay lines (e.g. tidyquant geom_ma × 2) — draw each as a
    // path with its own colour / dash style. Falls back to the ribbon+line
    // geom_smooth() default when GG_MA_LINES is not set.
    if (window.GG_MA_LINES) {
      window.GG_MA_LINES.forEach(function (line) {
        if (!line.data || !line.data.length) return;
        var d = '';
        line.data.forEach(function (p, i) { d += (i ? 'L' : 'M') + sx(p[0]) + ',' + sy(p[1]) + ' '; });
        var attrs = {
          d: d, fill: 'none',
        ...
        if (line.dashed) attrs['stroke-dasharray'] = '6,4';
        el('path', attrs, svg);
      });
      return;
    }
  ...
  function buildLabs(svg) {
    ...
    if (LABS.subtitle) {
      txt(svg, PX0, 42, LABS.subtitle,
          { 'font-size': 13, 'font-style': 'italic', fill: '#4d4d4d' });
    }
